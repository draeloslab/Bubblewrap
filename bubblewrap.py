...
class Bubblewrap:
    ...
    def observe(self, x, b=None):
        ...
        if not self.go_fast and self.obs.cov is not None and self.mu_orig is not None:
            ...
            self.sigma_orig = (
                self.obs.cov * (self.nu + self.d + 1) / (self.N ** (2 / self.d))
            )
    ...
    def teleport_node(self, x):
        ...
        if self.printing:
            print(
                "Teleported node ", node, " to current data location at time ", self.t
            )
    ...
    def grad_Q(self):
        ...
        (grad_mu, grad_L, grad_L_diag, grad_A) = self.grad_all(
            self.mu,
            self.L_lower,
            self.L_diag,
            self.log_A,
        ...
        self.run_adam(
            grad_mu / divisor, grad_L / divisor, grad_L_diag / divisor, grad_A / divisor
        )
    ...
    def run_adam(self, mu, L, L_diag, A):
        ...
        self.m_mu, self.v_mu, self.mu = single_adam(
            self.step, self.m_mu, self.v_mu, mu, self.t, self.mu
        )
        self.m_L, self.v_L, self.L_lower = single_adam(
            self.step, self.m_L, self.v_L, L, self.t, self.L_lower
        )
        self.m_L_diag, self.v_L_diag, self.L_diag = single_adam(
            self.step, self.m_L_diag, self.v_L_diag, L_diag, self.t, self.L_diag
        )
        self.m_A, self.v_A, self.log_A = single_adam(
            self.step, self.m_A, self.v_A, A, self.t, self.log_A
        )
...
@jit
def Q_j(
    mu, L_lower, L_diag, log_A, S1, lam, S2, n_obs, En, nu, sigma_orig, beta, d, mu_orig
):
    ...
    summed += (-1 / 2) * np.trace(
        (sigma_orig + S2 + lam * mus_orig + (lam + n_obs) * mus) @ sig_inv
    )
...
def update_internal(A, B, last_alpha, En, eps, S1, obs_curr, S2, n_obs):
    ...
    S2 = (1 - eps) * S2 + alpha[:, np.newaxis, np.newaxis] * (
        obs_curr[:, np.newaxis] * obs_curr.T
    )
        ...
        if not self.go_fast:
            ...
            if self.n_obs > 2:
                if self.cov is None:
                    ...
                    self.cov = update_cov(
                        self.cov, self.last_mean, self.curr, self.mean, self.n_obs
                    )
