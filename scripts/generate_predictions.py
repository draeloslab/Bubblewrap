import numpy as np
from bubblewrap import Bubblewrap, BWRun, NumpyPairedDataSource, AnimationManager, default_rwd_parameters, SymmetricNoisyRegressor
from bubblewrap.regressions import NearestNeighborRegressor
from bubblewrap.input_sources.data_sources import HMMSimDataSource, NumpyDataSource, PairWrapperSource, ConcatenatorSource, ProSVDDataSource
...
def main():
    for i in range(6):
        ...
        p_beh = ProSVDDataSource(beh.drop_time_offsets(), output_d=1, time_offsets=beh.time_offsets)
        c_both = ConcatenatorSource([obs, beh],)
        p3_both = ProSVDDataSource(ConcatenatorSource([obs.drop_time_offsets(), beh.drop_time_offsets()]), output_d=3, time_offsets=beh.time_offsets)
        p1_both = ProSVDDataSource(ConcatenatorSource([obs.drop_time_offsets(), beh.drop_time_offsets()]), output_d=1, time_offsets=beh.time_offsets)
        ...
        reg = SymmetricNoisyRegressor(input_d=bw.N, output_d=ds.output_shape[1], forgetting_factor=1 - (1e-2), noise_scale=1e-5)
        ...
        br = BWRun(bw=bw, data_source=ds, behavior_regressor=reg, show_tqdm=False, output_directory="/home/jgould/Documents/Bubblewrap/generated/bubblewrap_runs/")
